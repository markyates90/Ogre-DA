...
namespace DataAccessTests
{
	...
	public class CreationTests
	{
		...
		{
			ConnectionStringSettings connectionString = ConfigurationManager.ConnectionStrings["TestDBOleDb"];
			Database target = new Database(connectionString);
			Assert.IsInstanceOfType(target.CreateConnection(),
		...
		///</summary>
		[TestMethod()]
		public void DatabaseConstructorTest1OleDb()
		{
			string connectionName = "TestDBOleDb";
			Database target = new Database(connectionName);
			Assert.IsInstanceOfType(target.CreateConnection(),
				typeof(System.Data.OleDb.OleDbConnection),
				"Should be using OleDb types.");
		...
		{
			Database target = new Database("TestDBOleDb");
		...
		public void CreateDataAdapterTest()
		{
			...
			target = new Database("TestDBOleDb");
